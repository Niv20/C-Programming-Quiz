...
void printInBinary4(char n) {
  ...
  },
  {
    ...
    explanation:
      "הדרך הנכונה לבצע את ההחלפה היא באמצעות שלוש פעולות נפרדות שמאוחדות לבסוף. ראשית, יש לבודד כל קבוצת ביטים באמצעות מסכת `AND`, להזיז אותה למיקומה החדש, ולבסוף לאחד את כל החלקים עם `OR`. הפקודה הנכונה תהיה: `a = ((a & 0xE0) >> 4) | ((a & 0x1E) << 3) | (a & 0x01);`.\n\nאף אחת מהאפשרויות המוצעות אינה מבצעת את הפעולה כראוי. כולן מנסות להזיז את המספר `a` כולו (`x >> 5` או `x << 1`) מבלי לבודד תחילה את קבוצות הביטים. פעולת הזזה על המספר כולו גורמת לביטים מקבוצה אחת 'לגלוש' ולהשפיע על קבוצות אחרות, ובכך משבשת את התוצאה הסופית.\n\nמכיוון שכל הפקודות המוצעות משתמשות בלוגיקה שגויה של הזזת המספר השלם במקום הזזת קבוצות מבודדות, אף אחת מהן לא תניב את התוצאה הנדרשת.",
  },
  {
...
#define Q4(X) ((((X)^0x55555555)<<1)&0xAAAAAAAA) & ((X)^0xAAAAAAAA)`,
    ...
    correct: 0,
    hint: "התנאי שהמאקרו צריך לבדוק הוא שכל ביט זוגי הוא ההפך מהביט האי־זוגי שלידו. במקום לנתח את המאקרואים המסובכים, נסו לחשוב קודם מהי הדרך הפשוטה ביותר לממש את הבדיקה הזו. ישנם רק שני ערכים אפשריים שעונים על הדרישה.",
    explanation:
