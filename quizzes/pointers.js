...
void Alloc4(Student*** a, int *size) {
    *a = (Student*)malloc(*size * sizeof(Student**));
    ...
    question:
      "נתונים מבנה `Student` ומשתנה `arr` שהוא מערך של מצביעים ל־`Student`. איזו מבין הפונקציות שמשמאל תקצה את המערך `arr` בגודל `size` באופן תקין?",
    answers: [
      "Alloc3",
      "Alloc1",
      "Alloc2",
    ...
    correct: 0,
    hint: "כן, אני יודע שאיריס אמרה שלא יהיו שלוש כוכביות במבחן, אבל זו שאלה טובה, והקטע עם השלוש כוכביות ממש קטן. אז אם אתם מחפשים רמז: בחתימה של הפונקציה, צריך להיות כתוב `Student***`. והאמת שהרמז הזה לא ממש מועיל, כי בכל החתימות של הפונקציות גם ככה זה מופיע.\nלגבי שאר התיבות: חשבו מהו הטיפוס של כל איבר במערך שאתם מקצים, ומה הטיפוס של מצביע למערך כזה.",
    explanation:
      "כדי להקצות זיכרון עבור המשתנה `arr` (שהוא מסוג `Student**`) ולהחזיר את הכתובת דרך פרמטר, הפונקציה צריכה לקבל את כתובתו של `arr`, כלומר פרמטר מסוג `Student***`.\nהשורה המבצעת את ההקצאה צריכה להציב את הכתובת החדשה בתוך `arr` המקורי, ולכן עליה לבצע השמה ל־`*a`. אנו מקצים מערך של מצביעים לסטודנטים, לכן כל איבר במערך הוא מסוג `Student*`, וגודל ההקצאה הוא `*size * sizeof(Student*)`.\nהפונקציה `malloc` מחזירה `void*`, ויש להמיר את הכתובת לטיפוס של `*a`, שהוא `Student**`. הפונקציה היחידה שמבצעת את כל השלבים הללו נכונה היא `Alloc3`, עם השורה: `*a = (Student**)malloc(*size * sizeof(Student*));`",
  },
];
