const quizData = [
  ...
  {
    code: '#define F(a,b) (a + b*a)\n#define G(a,b,c,d) ((F(a,b)) < (F(c,d)) ? (F(a,b)) : (F(c,d)))\n\nvoid main()\n{\n    printf("%d\\n", G(1+2, 2, 3, 2));\n}',
    question: "מה הפלט של התכנית הבאה?",
    answers: ["7", "9", "4", "התכנית לא עוברת קומפילציה"],
    correct: 0,
    hint: "זכרו שפקודות מאקרו מבצעות החלפת טקסט פשוטה. הביטוי `1+2` יוצב כפי שהוא במקום הפרמטר המתאים. לאחר ההצבה, יש לחשב את ערך הביטוי הסופי לפי סדר קדימויות האופרטורים המוגדר בשפת ה־C.",
    explanation:
  ...
  },
  {
    ...
    correct: 0,
    hint: "השאלה מעט טריקית. שימו לב שבביטוי `a**b` במאקרו, התו `*` משמש לשתי מטרות שונות. הכוכבית הימנית היא אופרטור ה־dereference, הניגש לערך שהמצביע `b` מצביע עליו, בעוד הכוכבית השמאלית היא אופרטור הכפל בין אותו ערך לבין `a`.",
    explanation:
      "תחילה, ננתח את מצב המשתנים לפני הקריאה למאקרו. המערך `a` מאותחל לערכים `{5, 7}`. המצביע `c` מקבל את כתובת תחילת המערך (`&a[0]`), והפקודה `(*c)++` מגדילה את `a[0]` ל־`6`. לאחר מכן, המצביע `b` מקבל את הכתובת `c + 1`, ולכן הוא מצביע לאיבר השני במערך, `a[1]`, שערכו `7`.\n\nהקריאה למאקרו `mac(a[0], b)` גורמת להחלפת טקסט פשוטה. הביטוי `a**b` הופך ל־`a[0] * *b`. חשוב להבין את המשמעות הכפולה של התו `*` בהקשר זה: הכוכבית הראשונה (משמאל) היא אופרטור הכפל, בעוד הכוכבית השנייה היא אופרטור ה־dereference, הגוזר את הערך מהכתובת שהמצביע `b` מחזיק.\n\nלכן, הביטוי הסופי לחישוב הוא ערכו של `a[0]` (שהוא 6) כפול הערך שאליו `b` מצביע (שהוא `a[1]`, כלומר 7). התוצאה היא `6 * 7 = 42`.",
  },
  {
  ...
  {
    code: '#include <stdio.h>\n\n#define PRINT(i, limit) i++ < limit ? printf("1") : printf("0")\n\nvoid main()\n{\n    PRINT(0, 3);\n    PRINT(3, 0);\n}',
