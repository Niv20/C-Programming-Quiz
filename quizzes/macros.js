...
const quizData = [
{
  code: `#define INCREMENT(x) (x+1)
#define SQUARE(x) (x * x)

int main() {
    int num = 2;
    int res = SQUARE(INCREMENT(num+1));
    printf("%d\\n", res);
    return 0;
}`,
  question: "מה תדפיס התכנית הבאה?",
  answers: [
    "6",
  ...
  correct: 4,
  hint: "מאקרואים בשפת C הם הרחבת טקסט פשוטה שמתבצעת לפני הקומפילציה. נסו 'להריץ' את ההרחבה בראש: החליפו את SQUARE(...) בהגדרה שלו, ולאחר מכן החליפו את INCREMENT(...) בהגדרה שלו. שימו לב לסדר הפעולות שנוצר אחרי ההרחבה.",
  explanation: "הקדם־מעבד מחליף את המאקרואים טקסטואלית לפני שהקוד מתקמפל. נפרק את התהליך:\n1. הביטוי המקורי הוא `SQUARE(INCREMENT(num+1))`.\n2. המאקרו `SQUARE(x)` מוגדר כ־`x * x`. לכן, הביטוי הופך ל־`INCREMENT(num+1) * INCREMENT(num+1)`.\n3. כעת, נחליף את `num` בערכו (2): `INCREMENT(2+1) * INCREMENT(2+1)`.\n4. המאקרו `INCREMENT(x)` מוגדר כ־`x+1`. נחליף כל אחד מהם: `(2+1+1) * (2+1+1)`.\n5. בגלל סדר קדימויות פעולות חשבון, הביטוי הופך להיות `(4) * (4)`.\n6. התוצאה הסופית היא 16."
},
{
...
void main() {
    ...
    correct: 0,
    hint: "האמת שאני לא זוכר שראינו את זה בהרצאות, אבל זה כן הופיע במבחנים. אם רוצים לפצל פקודת מאקרו לשתי שורות, מוסיפים לסוף השורה הראשונה את התו `\\`. \n\nפקודת זכרו שפקודות מאקרו מבצעות החלפת טקסט פשוטה. הביטוי `1+2` יוצב כפי שהוא במקום הפרמטר המתאים. לאחר ההצבה, יש לחשב את ערך הביטוי הסופי לפי סדר קדימויות האופרטורים (מופיע בעמוד האחרון של דף הנוסחאות המעודכן)",
    explanation:
...
void main() {
    ...
    correct: 0,
    hint: "האמת שאני לא זוכר שראינו את זה בהרצאות, אבל זה כן הופיע במבחנים. אם רוצים לפצל פקודת מאקרו לשתי שורות, מוסיפים לסוף השורה הראשונה את התו `\\`. \n\nהיזכרו מה המשמעות של האופרטור `++` ועל אילו סוגי ערכים ניתן להפעיל אותו.",
    explanation:
